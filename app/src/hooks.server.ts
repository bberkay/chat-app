...
export async function handle({ event, resolve }: {event: any, resolve: any}): Promise<void>
{
    // TODO: Hook test edilecek.
     ...
     */
    let profile = event.cookies.get('profile') ? JSON.parse(event.cookies.get('profile')) : undefined;
    if (!profile)
    {
        if(get(usersStore).length === 0) throw new Error('No users found for using as default profile.');
        profile = get(usersStore)[0];
        event.cookies.set('profile', JSON.stringify(profile), { path: '/' });
    }
