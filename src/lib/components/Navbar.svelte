<script lang="ts">
    ...
    function changeTheme() {
        ...
        // Get variables from style root
        const root = document.documentElement;
        const bgColor = getComputedStyle(root).getPropertyValue(`--bg-${currentTheme}`);
        const textColor = getComputedStyle(root).getPropertyValue(`--text-${currentTheme}`);
        const frontColor = getComputedStyle(root).getPropertyValue(`--front-${currentTheme}`);
        const frontBrightColor = getComputedStyle(root).getPropertyValue(`--front-${currentTheme}-bright`);
        const borderColor = getComputedStyle(root).getPropertyValue(`--border-${currentTheme}`);
        const outlineColor = getComputedStyle(root).getPropertyValue(`--outline-${currentTheme}`);

        // Set variables to style root
        root.style.setProperty('--current-bg-color', bgColor);
        root.style.setProperty('--current-text-color', textColor);
        root.style.setProperty('--current-front-color', frontColor);
        root.style.setProperty('--current-front-bright-color', frontBrightColor);
        root.style.setProperty('--current-border-color', borderColor);
        root.style.setProperty('--current-outline-color', outlineColor);
    }
